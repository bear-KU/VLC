...
extern unsigned int signal_len;
// Role for VLC communication
extern char role[ROLE_SIZE];
// Callback method name
extern char callback_method[CALLBACK_METHOD_SIZE];
// VLC data
...
inline void send_notification(WiFiClient &client, JsonDocument &notification_doc);
inline void execute_callback(WiFiClient &client, const char* callback_method, const char* role);
...
// サーバからの JSON を処理
inline bool handle_json(WiFiClient &client, JsonDocument &doc) {
  // データが利用可能かチェック（ノンブロッキング）
  if (!client.available()) {
    return false; // データがなければすぐに false を返す
  }
...
// JSON-RPC 形式の通知をサーバに送信
inline void send_notification(WiFiClient &client, JsonDocument &notification_doc) {
  String notification;
  serializeJson(notification_doc, notification);
  client.println(notification);
  Serial.print("Sent notification to server: ");
...
inline void receive(WiFiClient &client, JsonDocument &request_doc) {
  ...
  // callback パラメータを保存
  JsonObject params = request_doc["params"];
  if (!params.isNull() && params.containsKey("callback")) {
    const char* callback = params["callback"];
    memset(callback_method, 0, sizeof(callback_method));
    strcpy(callback_method, callback);
  }
...
inline void execute_callback(WiFiClient &client, const char* callback_method, const char* role) {
  ...
  // サーバにコールバック通知を送信
  StaticJsonDocument<256> notification_doc;
  notification_doc["jsonrpc"] = "2.0";
  notification_doc["method"] = callback_method;
  notification_doc["params"]["role"] = role;
  
  send_notification(client, notification_doc);
  
  Serial.print("VLC communication completed for role: ");
  Serial.print(role);
  Serial.print(", executing callback: ");
  Serial.println(callback_method);
} // inline void execute_callback(WiFiClient &client, const char* callback_method, const char* role)
