...
inline String run_vlc_receive(int payload_size) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if(received == "ERROR") {
      Serial.println("No data received or communication timeout.");
      return received; // タイムアウトした場合は空文字を返す
    }

    data += received;
    // Serial.printf("Iteration %d: %s\r\n", i+1, received.c_str());
    delay(10); // 前回のトレイラの残りを読み込まないようにするため待つ
  }
...
String LED_read_binary(int payload_size, int threshold) {
  ...
  while(1) {
    ...
    if(prev_v == 0 && value != 0) {
      if((signal_unit * 0.5) < (time - edge_down_time)) {
        edge_up_time = time;
      } 
    }
